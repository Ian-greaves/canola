...
def filter_files(folder_path):
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith("PerGene.out.tab") and filename != "ReadsPerGene.out.tab":
...
def filter_rows_in_file(file_path):
    ...
    with open(file_path, 'r') as fr:
        for line in fr.readlines():
            if ("N_unmapped" in line or
                "N_multimapping" in line or
                "N_noFeature" in line or
                "N_ambiguous" in line
               ):
...
if __name__ == "__main__":
    ...
    save_dicts_to_file("filter_output1.txt", all_file_dict)


    with open("filter_output1.txt", "r") as f:
